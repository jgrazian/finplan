...
export function RecoveryForm({ onBack }: { onBack: () => void }) {
  ...
  return <form style={{ maxWidth: 400, margin: "72px auto", display: "grid", gap: 12 }} onSubmit={async (event) => {
    ...
    try {
      if (resetting) {
        await http.post<void>("/auth/reset-password", { token: token.trim(), new_password: password });
        ...
        setMessage("If that account exists, recovery instructions have been sent. Choose ‘I have a recovery token’ and paste the token from your email. The token expires after 30 minutes.");
      }
    ...
    {resetting ? <>
      <Field label="Recovery token from your email"><CompactInput value={token} onChange={(e) => setToken(e.target.value)} required autoComplete="off" spellCheck={false} autoCapitalize="none" /></Field>
